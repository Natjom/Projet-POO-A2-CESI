...
class Parametre3 : public BoutonParametres {
private:
    bool isClickedFlag = false; // Flag spécifique pour ce bouton

public:
    Parametre3(float x, float y, float width, float height, const std::string& text)
        : BoutonParametres(x, y, width, height, text) 
        {

            if (!font.loadFromFile("fonts/NeutronsDemoRegular.ttf")) {
    ...
    void onClick() override {
        ...
        if (!globalFont.loadFromFile("fonts/NeutronsDemoRegular.ttf")) {
            std::cerr << "Erreur : impossible de charger la nouvelle police." << std::endl;
        }
