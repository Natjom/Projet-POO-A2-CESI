...
class Parametre1 : public BoutonParametres {
private:
    bool isClickedFlag = false; // Flag spécifique pour ce bouton

public:
    Parametre1(float x, float y, float width, float height, const std::string& text)
    ...
    void onClick() override {
        ...
        if (!globalFont.loadFromFile("fonts/Minecraft.ttf")) {
            std::cerr << "Erreur : impossible de charger la nouvelle police." << std::endl;
        }
